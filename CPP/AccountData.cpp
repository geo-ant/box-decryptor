...
AccountData::AccountData(std::filesystem::path const &keyfilePath,
                         std::string const &password) {
  this->ParseBCKeyFile(keyfilePath);
...
bool AccountData::ParseBCKeyFile(const std::filesystem::path &keyfilePath) {
  std::cout << "Parsing .bckey file: '" << keyfilePath << "'" << std::endl;

  if (keyfilePath.extension() != ".bckey") {
    throw std::runtime_error(
        "Given filepath does not have the right extension ('.bckey'), please "
        "specify a Boxcryptor key file");
  }

  std::ifstream keyFile(keyfilePath, std::ios::binary | std::ios::ate);
  if (!keyFile.good()) {
    std::string errorMsg(
        ".bckey-File (" + keyfilePath.string() +
  ...
  if (posBegin > posUserEnd) {
    throw std::runtime_error(
        "The (first) user object has no suitable 'privateKey' value");
  }
  size_t posEnd = keyFileData.find(R"(")", posBegin + 1);

  if (posBegin != std::string::npos && posEnd != std::string::npos &&
      posEnd - posBegin > 0) {
    this->m_encryptedPrivateKey =
        keyFileData.substr(posBegin + 1, posEnd - posBegin - 1);
  } else {
  ...
  if (posBegin > posUserEnd) {
    throw std::runtime_error(
        "The (first) user object has no suitable 'salt' value");
  }
  ...
  if (posBegin > posUserEnd) {
    throw std::runtime_error(
        "The (first) user object has no suitable 'kdfIterations' value");
  }
  ...
  if (posBegin != std::string::npos && posEnd != std::string::npos &&
    ...
    std::string iterations =
        keyFileData.substr(posBegin + 1, posEnd - posBegin - 1);
    try {
      this->m_pbkdf2Iterations = std::stoi(iterations);
    } catch (...) {
      throw std::runtime_error("Could not convert iterations value to integer");
    }
  } else {
...
std::string AccountData::GetPassword() const { return this->m_password; }

std::string AccountData::GetPBKDF2Salt() const { return this->m_pbkdf2Salt; }
