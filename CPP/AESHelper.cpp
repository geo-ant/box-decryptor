...
bool AESHelper::DecryptDataPBKDF2(const std::string &data,
  ...
  if (pbkdf2Password.length() > 0 && pbkdf2Salt.length() > 0 &&
    ...
    auto cryptoKey =
        std::vector<byte>(hashBytes.begin(), hashBytes.begin() + 32);
    ...
    auto IVec = std::vector<byte>(decodedPrivateKeyBytes.begin(),
                                  decodedPrivateKeyBytes.begin() + 16);
    auto givenHmacHash = std::vector<byte>(decodedPrivateKeyBytes.begin() + 16,
                                           decodedPrivateKeyBytes.begin() + 48);
    auto privateKeyBytes = std::vector<byte>(
        decodedPrivateKeyBytes.begin() + 48, decodedPrivateKeyBytes.end());
    ...
    if (givenHmacHash != computedHmacHash) {
      throw std::runtime_error("HMAC hashes do not match, make sure you used a "
                               "matching .bckey file and password");
    }
    ...
    throw std::runtime_error(
        "Password and salt for the PBKDF2 algorithm can not be empty and the "
...
// from
// https://github.com/vgough/encfs/blob/559c30d01ed0a3d19258b12f15eae8785accc60f/encfs/SSL_Cipher.cpp#L626
/*private*/ std::vector<byte>
AESHelper::ComputeBlockIVec(std::vector<byte> IVec, unsigned long seed,
                            std::vector<byte> key) {
...
/*private*/ bool AESHelper::DecryptData(
    ...
    CryptoPP::BlockPaddingSchemeDef::BlockPaddingScheme
        paddingMode /* = CryptoPP::StreamTransformationFilter::PKCS_PADDING*/) {
  if ((isUserGeneratedData || data.size() > 0) && cryptoKey.size() > 0 &&
      IVec.size() > 0) {
    ...
    throw std::runtime_error(
        "Encrypted data, crypto key and initialization vector can't be empty");
  }
}
