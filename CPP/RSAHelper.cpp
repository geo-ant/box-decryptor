...
RSAHelper::DecryptFileKey(const std::string &encryptedFileKey,
  ...
  if (decryptedPrivateKey.size() > 0) {
    ...
    // private key is stored in a simplified PEM format (no header/footer and no
    // line breaks) decode it from base 64 again to get the DER encoding needed
    // by Crypto++
    ...
    decryptedFileKey.resize(
        rsaDecryptor.MaxPlaintextLength(decodedFileKey.size()));
    ...
    auto result = rsaDecryptor.Decrypt(rng, encryptedKey, decodedFileKey.size(),
                                       decryptedKey);
    ...
    throw std::runtime_error(
        "The private key used for the RSA decryption can't be of length 0");
  }
}
