...
PBKDF2Helper::PBKDF2Helper(std::string pwd, std::vector<byte> salt,
                           int iterations = 5000)
    : m_iterations(iterations), m_salt(salt) {
  this->m_password = std::vector<byte>(pwd.begin(), pwd.end());
}

bool PBKDF2Helper::GetBytes(unsigned int count,
                            std::vector<byte> &derivedBytes) {
  /* std::cout << "PBKDF2 algorithm to get " << count << " bytes started" <<
   * std::endl; */
  ...
  if (count > 0) {
    ...
    try {
      pbkdf2.DeriveKey(derived, count, 0, pwd, this->m_password.size(), salt,
                       this->m_salt.size(), this->m_iterations);
